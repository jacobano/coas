 ...
 *
 * Geographic to cartesian coordinate conversion, with an intermediate conversion to UTM.
 * 
 * This library suppose that the path of the robot cannot cross more than 2 different UTM zones.
 * 
 */

#ifndef GEOGRAPHIC_TO_UTM_TO_CARTESIAN
#define GEOGRAPHIC_TO_UTM_TO_CARTESIAN
...
inline geometry_msgs::Point32 geographic_to_UTM_to_cartesian(geographic_msgs::GeoPoint actual_coordinate_geo, geographic_msgs::GeoPoint origin_geo)
{
    geodesy::UTMPoint actual_coordinate_UTM(actual_coordinate_geo); // Conversion from geographic coordinates to UTM.
    geodesy::UTMPoint origin_UTM(origin_geo);                       // Conversion from geographic coordinates to UTM.
    ...
    if (int(actual_coordinate_UTM.zone) == 60 && origin_UTM.zone == 1)
    { // Coordinate and origin separated by the +-180º longitude
        geographic_msgs::GeoPoint geo_180_w;
        geographic_msgs::GeoPoint geo_180_e;
        geo_180_w.longitude = 179.9999999;
        geo_180_w.latitude = actual_coordinate_geo.latitude;
        geo_180_w.altitude = actual_coordinate_geo.altitude;
        geo_180_e.longitude = -179.9999999;
        geo_180_e.latitude = actual_coordinate_geo.latitude;
        geo_180_e.altitude = actual_coordinate_geo.altitude;
    ...
    else if (origin_UTM.zone == 60 && int(actual_coordinate_UTM.zone) == 1)
    { // Coordinate and origin separated by the +-180º longitude
        geographic_msgs::GeoPoint geo_180_w;
        geographic_msgs::GeoPoint geo_180_e;
        geo_180_w.longitude = 179.9999999;
    ...
    {
        int quotient_from_int_division = (int)(std::max(std::abs(actual_coordinate_geo.longitude), std::abs(origin_geo.longitude)) / 6);                     // int division of the max longitude (absolute, without sign)
        double border_longitude = quotient_from_int_division * 6.0 * pow(-1, std::signbit(std::max(actual_coordinate_geo.longitude, origin_geo.longitude))); // border_longitude = quotient_from_int_division * 6 *(-1)^(1 if negative longitude, 0 if positive)
        geographic_msgs::GeoPoint geo_w;
        geographic_msgs::GeoPoint geo_e;
        geo_w.longitude = border_longitude - 0.0000001;
        geo_w.latitude = actual_coordinate_geo.latitude;
        geo_w.altitude = actual_coordinate_geo.altitude;
        geo_e.longitude = border_longitude + 0.0000001;
        geo_e.latitude = actual_coordinate_geo.latitude;
        geo_e.altitude = actual_coordinate_geo.altitude;
    ...
    if (origin_UTM.band == 'N' && actual_coordinate_UTM.band == 'M')
    { // Coordinate and origin separated by the 0º latitude
        geographic_msgs::GeoPoint geo_0_n;
        geographic_msgs::GeoPoint geo_0_s;
        geo_0_n.longitude = actual_coordinate_geo.longitude;
        geo_0_n.latitude = 0.0000001;
        geo_0_n.altitude = actual_coordinate_geo.altitude;
        geo_0_s.longitude = actual_coordinate_geo.longitude;
        geo_0_s.latitude = -0.0000001;
        geo_0_s.altitude = actual_coordinate_geo.altitude;
    ...
    else if (actual_coordinate_UTM.band == 'N' && origin_UTM.band == 'M')
    { // Coordinate and origin separated by the 0º latitude
        geographic_msgs::GeoPoint geo_0_n;
        geographic_msgs::GeoPoint geo_0_s;
        geo_0_n.longitude = actual_coordinate_geo.longitude;
